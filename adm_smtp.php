...
if($save_kw){
	...
	$sql="update smtp set txt='".$txt."' where id=$id_kw";
	$conn->query($sql);
	$id_kw=0;
}
...
<h3><i class="fas fa-bars"></i> <? echo $kw["set_mail"];?></h3>
...
<tbody>
<?
$sql="select id, kod, kod_name, txt from smtp";
$res=$conn->query($sql);
while($row=$res->fetch_array()){
	...
	<td class="align-middle">
		<h6><?=$row['kod'];?></h6>
		<span style="font:12px Arial;color:#555;"><?=$row["kod_name"];?></span>
	</td>
	<td class="align-middle">
		<?=$row['txt'];?>
	</td>
...
if($tag==2){
	$txt='';
	if($id_kw>0){
		$data=getrec('smtp','kod,kod_name,txt',"where id=$id_kw");
		$kod=$data["kod"];
	...
	<div class="form-group row">
		...
		<div class="col-sm">
			<textarea id="txt" class="form-control form-control-sm" name="txt" rows="3" ><?=$txt;?></textarea>
		</div>
	</div>
