<?php
/*
$config['smtp_username'] = 'mu@bank-ombudsman.kz';  //Смените на адрес своего почтового ящика.
$config['smtp_port'] = '25'; // Порт работы.
$config['smtp_host'] =  'bank-ombudsman.kz';  //сервер для отправки почты
$config['smtp_password'] = 'gauhar';  //Измените пароль
$config['smtp_debug'] = true;  //Если Вы хотите видеть сообщения ошибок, укажите true вместо false
$config['smtp_from'] = 'MU'; //Ваше имя - или имя Вашего сайта. Будет показывать при прочтении в поле "От кого"
*/
...
?>
<?php
//smtpmail('Mu', '25338@mail.ru', 'Тема письма проверка', 'Проверка еще раз ');
$acopy=explode(',', $config['smtp_copy']);
foreach ($acopy as $key => $value) {
	$value=trim($value);
	if($value){
		smtpmail($kw["from_name"], $value, $subj, $mess);
	}
}
?>